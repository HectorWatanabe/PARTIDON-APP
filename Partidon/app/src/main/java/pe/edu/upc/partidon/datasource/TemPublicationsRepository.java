...
public class TemPublicationsRepository {

    private final PartidonService service;
    private Context context;

    public TemPublicationsRepository(Context context){
    ...
    public interface TeamPublicationsCallback{
        void onComplete(List<NewsComments> publications);
        void onError(String message);
    }

    public void getTeamPublications(int id,final TemPublicationsRepository.TeamPublicationsCallback callback){

        SharedPreferences references = getDefaultSharedPreferences();
        service.getTeamPublications(id,references.getString("api_token",null)).enqueue(new Callback<List<NewsComments>>() {
            @Override
