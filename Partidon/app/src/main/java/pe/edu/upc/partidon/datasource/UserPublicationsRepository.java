...
public class UserPublicationsRepository {

    private final PartidonService service;
    private Context context;

    public UserPublicationsRepository(Context context){
    ...
    public interface UserPublicationsCallback{
        void onComplete(List<NewsComments> publications);
        void onError(String message);
    }

    public void getUserPublications(int id,final UserPublicationsRepository.UserPublicationsCallback callback){

        SharedPreferences references = getDefaultSharedPreferences();
        service.getUserPublications(id,references.getString("api_token",null)).enqueue(new Callback<List<NewsComments>>() {
            @Override
